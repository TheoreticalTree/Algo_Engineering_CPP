...
inline std::vector<edge> createValidMSTInstanceWithIntegerWeights(uint64_t n, int targetEdgeNumber, int maximumWeight, int seed)
{
    bool edgeIsDouble = true;
    uint64_t maxEdgeNumber = n * (n - 1) / 2;
    if (targetEdgeNumber > maxEdgeNumber || targetEdgeNumber < n - 1)
    ...
    std::vector<edge> edgelist;
    std::mt19937 gen(seed);
    std::uniform_int_distribution<> randomWeight(0, maximumWeight);;
    srand(seed);
    int i, j, temp, sign;
    double random;
...
// inline std::vector<edge> createValidCliqueInstance(uint64_t n, int targetEdgeNumber, double maximumWeight, int seed, int max_num_neighbours) {
//     bool edgeIsDouble = true;
//     uint64_t maxEdgeNumber = n * (n - 1) / 2;
//     if (targetEdgeNumber > maxEdgeNumber || targetEdgeNumber < n - 1)
//     {
...
//     }
//
//     std::vector<int> num_neighbours(n);
//     for(int i=0;i<edgelist.size();i++) {
//         int v = edgelist[i].v;
//         int w = edgelist[i].w;
//         if(num_neighbours[v]==max_num_neighbours || num_neighbours[w]==max_num_neighbours) {
//             edgelist[i] = edgelist[edgelist.size() - 1];
//             edgelist.pop_back();
//         } else {
//             num_neighbours[v]++;
//             num_neighbours[w]++;
